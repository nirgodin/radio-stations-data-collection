...
class TranslationAdapter:
    ...
    async def translate(
        ...
        translation = await self._retrieve_translation_from_cache(
            text=text, target_language=target_language, filters=filters
        )
    ...
    async def _retrieve_translation_from_cache(
        self, text: str, target_language: str, filters: Dict[Type[Translation], Any]
    ) -> Optional[str]:
    ...
    async def _insert_new_translation_record(
        self,
        text: str,
        target_language: str,
        translation: Optional[str],
        filters: Dict[Type[Translation], Any],
    ) -> None:
        if translation is None:
