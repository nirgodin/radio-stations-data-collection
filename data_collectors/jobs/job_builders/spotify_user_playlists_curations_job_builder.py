...
class SpotifyUserPlaylistsCurationsJobBuilder(BaseJobBuilder):
    async def build(self, next_run_time: Optional[datetime] = undefined) -> ScheduledJob:
        return ScheduledJob(
            task=self._task,
            id=JobId.SPOTIFY_USER_PLAYLISTS_CURATIONS,
    ...
    async def _task(self) -> None:
        async with self._component_factory.sessions.enter_spotify_session() as spotify_session:
            manager = self._component_factory.curations.get_spotify_user_playlists_curations_manager(spotify_session)
            await manager.run()
