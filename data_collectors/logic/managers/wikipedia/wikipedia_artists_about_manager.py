...
class WikipediaArtistsAboutManager(IManager):
    ...
    async def _retrieve_artists_raw_details(
        ...
        query = (
            ...
            .where(
                and_(
                    SpotifyArtist.wikipedia_name.isnot(None),
                    SpotifyArtist.wikipedia_language.isnot(None),
                )
            )
    ...
    async def _collect_artist_wiki_summary(
        self, artist_details: WikipediaArtistAbout
    ) -> Optional[WikipediaArtistAbout]:
    ...
    @staticmethod
    async def _insert_single_summary_document(
        artist_details: WikipediaArtistAbout,
    ) -> bool:
