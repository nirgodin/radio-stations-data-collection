...
class BaseWikipediaAgeCollector(ABC):
    ...
    async def collect(self, limit: Optional[int]) -> Dict[str, str]:
        logger.info(
            f"Starting to collect artists age details using `{self.__class__.__name__}` collector"
        )
    ...
    async def _collect_single_artist_summary(
        self, artist_id_and_detail: str
    ) -> Tuple[str, str]:
        ...
        summary = await self._page_summary_collector.collect(
            artist_page_name, wikipedia_abbreviation
        )
