...
from data_collectors.consts.billboard_consts import (
    ...
    BILLBOARD_DAILY_CHARTS_URL_FORMAT,
)
...
class BillboardChartsCollector(ICollector):
    def __init__(
        self, session: ClientSession, pool_executor: AioPoolExecutor = AioPoolExecutor()
    ):
    ...
    async def _collect_single_date_charts(self, date: datetime) -> ChartData:
        ...
        url = BILLBOARD_DAILY_CHARTS_URL_FORMAT.format(
            name=BillboardChart.HOT_100.value, date=formatted_date
        )
    ...
    @staticmethod
    def _create_chart_data_from_text(
        chart_page_text: str, formatted_date: str
    ) -> ChartData:
        chart_data = ChartData(
            name=BillboardChart.HOT_100.value, date=formatted_date, fetch=False
        )
