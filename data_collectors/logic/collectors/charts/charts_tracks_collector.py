...
class ChartsTracksCollector(ICollector):
    ...
    async def collect(self, charts_entries: List[ChartEntry]) -> List[RadioChartEntryDetails]:
    ...
    async def _create_chart_entry_details_by_track_id(self, chart_entry: ChartEntry) -> RadioChartEntryDetails:
        ...
        logger.info(f"Did not find any track matching `{chart_entry.track_id}` from date `{string_date}`. Ignoring")
