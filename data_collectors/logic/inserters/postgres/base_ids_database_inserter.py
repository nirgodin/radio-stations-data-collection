...
class BaseIDsDatabaseInserter(IPostgresDatabaseInserter, ABC):
    async def insert(self, iterable: Iterable[Any]) -> List[BaseORMModel]:
        ...
        records = [
            getattr(self._orm, self._serialization_method)(record)
            for record in raw_records
        ]
        valid_records = [
            record for record in records if isinstance(record, BaseORMModel)
        ]
    ...
    async def _query_existing_ids(self, records: List[BaseORMModel]) -> List[str]:
        logger.info(
            f"Querying database to find existing ids for table `{self._orm.__tablename__}`"
        )
        existing_ids = await query_existing_column_values(
            orm=self._orm, column_name=ID, records=records, engine=self._db_engine
        )
        logger.info(
            f"Found {len(existing_ids)} existing record in table `{self._orm.__tablename__}`"
        )

        return existing_ids

    async def _insert_non_existing_records(
        self, records: List[BaseORMModel], existing_ids: List[str]
    ) -> None:
        ...
        if non_existing_records:
            logger.info(
                f"Inserting {len(non_existing_records)} records to table {self._orm.__tablename__}"
            )
            await insert_records_ignoring_conflicts(
                engine=self._db_engine, records=non_existing_records
            )

        self._log_summary(records, non_existing_records)

    def _log_summary(
        self, records: List[BaseORMModel], non_existing_records: List[BaseORMModel]
    ) -> None:
        ...
        logger.info(
            f"Found {n_exist} existing records and {n_non_exist} non existing records in table `{table_name}`"
        )
