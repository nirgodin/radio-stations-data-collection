...
class MongoChunksDatabaseInserter(IDatabaseInserter):
    ...
    async def insert(
        self, records: List[BaseDocument], model: Type[BaseDocument]
    ) -> None:
        logger.info(
            f"Inserting {len(records)} by chunks to MongoDB `{model.__name__}` collection"
        )
        await self._chunks_generator.execute_by_chunk_in_parallel(
            lst=records, func=model.insert_many, expected_type=InsertManyResult
        )
