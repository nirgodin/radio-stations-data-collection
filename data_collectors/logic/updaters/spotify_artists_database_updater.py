...
class SpotifyArtistsDatabaseUpdater(BaseDatabaseUpdater):
    ...
    async def update(self, update_requests: List[ArtistUpdateRequest]) -> None:
    ...
    async def _update_single_artist_gender(self, update_request: ArtistUpdateRequest) -> None:
        await self._update_by_values(
            SpotifyArtist,
            update_request.values,
            SpotifyArtist.id == update_request.artist_id
        )
