...
@fixture(scope='session')
async def spotify_test_client(spotify_credentials: ClientCredentials) -> SpotifyTestClient:
...
@fixture(scope='session')
def env_component_factory(spotify_credentials: ClientCredentials,
                          spotify_test_client: SpotifyTestClient,
                          postgres_testkit: PostgresTestkit) -> EnvironmentComponentFactory:
    default_env = {
        ...
        "SPOTIPY_TOKEN_REQUEST_URL": spotify_test_client._authorization_server.url_for("").rstrip("/"),  # TODO: Externalize authorization server url
...
@fixture(scope='session')
def component_factory(env_component_factory: EnvironmentComponentFactory) -> ComponentFactory:
